package com.store.demo.keycloak.pure_client;
...
public class PureClientApplication {
	...
	@Bean
	CommandLineRunner run(OAuth2AuthorizedClientManager manager, RestTemplate rest,
			@Value("${resource-server.url}") String resourceServerUrl) {
		return _ -> {
			var authRequest = OAuth2AuthorizeRequest.withClientRegistrationId("keycloak-client").principal("machine")
					.build();
			...
			var resp = rest.exchange(resourceServerUrl + "/data", HttpMethod.GET, new HttpEntity<>(headers),
					String.class);

			System.out.println("Response from resource server: " + resp.getBody());
